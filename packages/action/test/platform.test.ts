...
test("reads the dispatch context the platform sends", () => {
  const env = eventFile({
    ...
    client_payload: { auditRunId: "au-1", token: "tok", resultUrl: "https://x.test/r" },
  });
  assert.deepEqual(readDispatchContext(env), {
    auditRunId: "au-1",
    token: "tok",
    resultUrl: "https://x.test/r",
...
test("a partial client_payload reports nothing rather than guessing", () => {
  for (const partial of [
    ...
    { auditRunId: "au-1", resultUrl: "https://x.test/r" },
    { token: "tok", resultUrl: "https://x.test/r" },
    { auditRunId: "", token: "tok", resultUrl: "https://x.test/r" },
  ]) {
...
test("a scored check reports its score and findings", () => {
  const r = scoredResult("scan", "qScan", 66, [{ ruleId: "x", severity: "high", title: "t" }]);
...
test("posts the payload with the run id and token merged in", async () => {
  ...
  const ok = await postResult(
    { auditRunId: "au-9", token: "tok-9", resultUrl: "https://x.test/r", eventType: null },
    scoredResult("scan", "qScan", 100, []),
  ...
  assert.ok(ok);
  assert.equal(seen!.url, "https://x.test/r");
...
test("a failed post is reported, never thrown", async () => {
  ...
  const ctx = { auditRunId: "a", token: "t", resultUrl: "https://x.test/r", eventType: null };
