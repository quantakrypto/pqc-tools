...
| `disabledRules` | string[] | `[]` | `ScanOptions.disabledRules` | Rule ids to suppress (e.g. `"node-crypto-ecdh"`, `"tls-weak-cipher"`). Finer-grained than `detectors.<family>`. See the catalog via the MCP `list_rules` tool (or the `quantakrypto://rules` resource). Unknown ids are harmless (never match). |
