...
in code**; F1/F2/F3 have since been **fixed** (2026-07-20 — blast-radius gate,
provider-`system`-role instruction/data separation, and a `--max-findings` spend
cap); the residual documented gap is **F5** (advisory MCP gates — see §6.5).

| STRIDE | Threat | Verdict (2026-07-15 audit) | Severity |
|---|---|---|---|
| **T**amper | **Remediation patch is only *crypto-count*-verified.** The `verify_fix` gate (`remediate-pipeline.ts:42-46`) passes when the target rule is gone + no new *crypto* rule types + fewer total findings. It says nothing about the rest of a **full-file LLM rewrite**, so an injected/hostile model can add arbitrary non-crypto code (e.g. an exfil `fetch(...process.env...)`) and pass; **`--mode apply` writes it unreviewed** (`remediate-cli.ts:289`). | **FIXED (F1).** A blast-radius gate now caps an LLM patch at `LLM_PATCH_MAX_CHANGED_LINES` (`remediate-pipeline.ts`); the output framing says "crypto-verified, not security-reviewed" and directs the operator to review the diff. Worktree isolation, patch-policy scope, LLM-path discard, and no-auto-merge already **held**. | low (fixed) |
| **T**amper | **Prompt injection → triage suppression.** Attacker text in scanned code tells the model to mark a real finding a false positive. | **HELD.** Exit code is computed from raw severities *before* triage runs (`qscan/src/index.ts:207-213`); `runTriage` only annotates/re-ranks — it never removes a finding. Triage **cannot** suppress or flip CI. **F2 fixed**: the rubric now travels in the provider's real `system` role with an explicit anti-injection preamble (`agent/src/loop.ts`), out-ranking the untrusted user turn. Residual: machine-output reorder + attacker `rationale` text (**F6**, low). | low (held) |
...
| **D**oS | **Budget exhaustion** — a repo with thousands of findings drives one paid call each (×2 with repair retry). | **FIXED (F3).** `--max-findings` caps the number triaged (`qscan/src/triage-run.ts`, `DEFAULT_MAX_TRIAGE`); when capping, the top findings by severity are chosen, so the spend is bounded per run. | low (fixed) |
